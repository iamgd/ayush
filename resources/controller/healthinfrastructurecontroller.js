var healthinfrastructurecontroller = moadashboardapp.controller('healthinfrastructurecontroller', function($scope, $http, commondata, $rootScope) {

    $scope.heading = "Ayush Health Infrastructure";
    ...
    $http(parameter3).then(function(response) {
        $scope.facility_list = (response.data);
        //alert($scope.facility_list);
        //$scope.selectedstate0=$scope.state_array0[0];
    });
    <!-- ------------State Dropdown Over --------------------- -->
    <!-- ------------- Options -------------------------------- -->
    $scope.addressdetail = function($index) {

        alert(JSON.stringify($scope.facility_list[$index]));
        var parameter = {
            method: 'POST',
            url: 'facilitydetailservlet',
            data: $scope.facility_list[$index],
            'Content-Type': "application/json;charset=utf-8"
        };
        $http(parameter).then(function(response) {
            $scope.facility_list = response.data;
            //$scope.AIH_list=(response.data);
            alert($scope.facility_list);
        }, function(error) {});
    }
